...
class AgileStoreLocator_Helper {
  ...
  public static function get_closest_store($origLat, $origLon, $dist) {
    ...
    $query = "SELECT id,title, lat, lng, email,phone, 3956 * 2 * 
              ...
              *POWER(SIN(($origLon-lng)*pi()/180/2),2))) 
              as distance FROM {$prefix}stores WHERE 
  ...
  public static function backup_template($template_id) {
    ...
    //  Validate the name
    if(!$template_name) {
      return ['success' => false, 'msg' => esc_attr__('Error! Template file is not valid.', 'asl_admin')];
    }
    ...
    if (@copy($template_file_path, $theme_directory.'/'.$template_name)) {
      ...
      return ['success' => true, 'msg' => esc_attr__('Template file moved to theme directory.', 'asl_admin')];
    }

    return ['success' => false, 'msg' => esc_attr__('Error! fail to move the file, check permisions.', 'asl_admin')];
  }
  ...
  public static function remove_template($template_id) {
    ...
    if(!file_exists($theme_directory.'/'.$template_name)) {
      ...
      return ['success' => false, 'msg' => esc_attr__("Error! Template file doesn't exist.", 'asl_admin')];
    }
    ...
    if (@unlink($theme_directory.'/'.$template_name)) {
      ...
      return ['success' => true, 'msg' => esc_attr__('Template file removed from the theme directory.', 'asl_admin')];
    }
  ...
  public static function removeFile($file, $directory) {
    ...
    if(file_exists($file_path)) { 
      ...
      $response->msg      = esc_attr__('File deleted successfully.','asl_admin');
    }
    else
      $response->error = esc_attr__('Error! fail to delete the file.','asl_admin');
