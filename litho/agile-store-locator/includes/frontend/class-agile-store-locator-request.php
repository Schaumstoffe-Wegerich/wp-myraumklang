 ...
 */
class AgileStoreLocator_Request {
    ...
    if($category) {
      ...
      $special = implode(',', $special);
		...
		if($slug_link) {
			...
			$rewrite_config = AgileStoreLocator_Helper::get_configs(['rewrite_slug', 'rewrite_id']);
	...
	private function add_test_stores() {
		...
		$file_ = '/home/dev/projects/wordpress_language/aronia.json';
	...
	/**
	 * [_store_register_notification Send the notification to the owner about registeration of the new store]
	 * @param  [type] $form_data [description]
	 * @return [type]            [description]
	 */
	private function _store_register_notification($form_data) {

		global $wpdb;

		$all_configs = AgileStoreLocator_Helper::get_configs(['admin_notify', 'notify_email']);
		...
		if(isset($all_configs['admin_notify']) && $all_configs['admin_notify'] == '1') {
			...
			if($admin_email) {
			  ...
			  //	Send a email notification
				AgileStoreLocator_Helper::send_email($admin_email, $subject, $message);
			}
    ...
    $response->success 	= false;
		...
		$msg_text 	 = strip_tags(sanitize_text_field($lead_details['message']));
 		...
 		$coordinates = AgileStoreLocator_Helper::getCoordinates('', '', '', $postal_code, $lead_country, AgileStoreLocator_Helper::get_configs('server_key'));


 		//2- Calc the closest Store to the fetched zipcode, 2000 miles
		$closest_store = AgileStoreLocator_Helper::get_closest_store($coordinates['lat'], $coordinates['lng'], $area_radius);
		...
		$mail_configs = AgileStoreLocator_Helper::get_configs(['admin_notify', 'notify_email']);
 		...
 		if(($closest_store && $closest_store->email) || $mail_configs['notify_email']) {
			...
			if($wpdb->insert(ASL_PREFIX."leads", array(
				...
				if($mail_configs['admin_notify']) {
				  ...
				  //	Send a email notification
					$response->s = AgileStoreLocator_Helper::send_email($to_email, $subject, $content, $headers);
				}
	...
	//	Get the fields data
		...
		//	Get the store
		$selected_store = AgileStoreLocator_Helper::get_store(intval($store_id));
	...
	public function register_store() {
      ...
      'updated_on' 	=> date('Y-m-d H:i:s')
		...
		if($wpdb->insert(ASL_PREFIX."stores", $store_attrs)) {
			...
			$this->_store_register_notification($store_attrs);
