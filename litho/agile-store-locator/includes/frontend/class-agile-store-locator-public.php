...
class AgileStoreLocator_Public {
	...
	public function enqueue_styles($template = '') {
		...
		switch ($template) {
			...
			case 'list':
				...
				//	Bootstrap, todo remove with sl-bootstrap
  ...
  /**
   * [initBorlabCookies use Borlab Cookies if plugin is installed]
   * @return [type] [description]
   */
  public function initBorlabCookies() {

    if (function_exists('BorlabsCookieHelper')) {
      
      require_once ASL_PLUGIN_PATH.'includes/class-store-locator-borlab.php';

      $borlab = new AgileStoreLocator_Borlab;

      $borlab->initialize();

      //add_action('init', array($borlab, 'initialize'));
    }
  }
    ...
    /*Brand*/
    $tmp_data       = $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."brands WHERE lang = '$lang' ORDER BY name ASC");
    ...
    /*Special*/
    $tmp_data       = $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."specials WHERE lang = '$lang' ORDER BY name ASC");
    ...
    $markers   	= $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."markers");
		...
		$fields = AgileStoreLocator_Helper::get_custom_fields();
	...
	public function storePage($atts) {
        ...
        if(isset($store_data->categories) && $store_data->categories) {
          ...
          if($categories) {
		    ...
		    $all_configs = AgileStoreLocator_Helper::get_configs(['store_schema', 'zoom', 'map_layout']);
    ...
    if(isset($atts['brand'])) {
      ...
      $where_brand  				= ' AND id = '.$all_configs['brand'];
		...
		if(isset($atts['kml']) && $atts['kml'] == '1') {
			...
			//	Get the KML files
			$kml_files = AgileStoreLocator_Helper::get_kml_files();
		...
		*/
		...
		/*Brand*/
		$tmp_data 			= $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."brands WHERE lang = '$lang' {$where_brand} ORDER BY name ASC");
		...
		/*Special*/
		$tmp_data 			= $wpdb->get_results("SELECT id, name FROM ".ASL_PREFIX."specials WHERE lang = '$lang' {$where_special} ORDER BY name ASC");
		...
		*/
		...
		$css_code .= AgileStoreLocator_Helper::generate_tmpl_css($all_configs['template']);
    ...
    $asl_tmpls = AgileStoreLocator_Helper::get_template_views($all_configs['template']);
  ...
  private function have_matching_address($zip_code) {
  	...
  	$selected_store = AgileStoreLocator_Helper::get_store(null, "s.description_2 LIKE '%".$wpdb->esc_like( $zip_code ) ."%'");
